        ...
        if (isset($_POST['submit'])) {
            ...
            if (isset($_POST['selectedAddresses']) && is_array($_POST['selectedAddresses']) && !empty($_POST['selectedAddresses'])) {
                ...
                foreach ($order_ids as $order_id) {
                    ...
                    $stmt->bind_param("iii", $addressId, $user_id, $order_id); // Updated from $order_ids
                    $stmt->execute();
                    $stmt->close();
                }
...
$username = "";
if (!empty($user_id)) {
    $getUserSql = "SELECT username FROM users WHERE id = ?";
    $getUserStmt = $conn->prepare($getUserSql);
    $getUserStmt->bind_param("i", $user_id);
    $getUserStmt->execute();
    $getUserStmt->bind_result($fetchedUsername);
...
<body>
    <div class="dash">
        ...
        <div class="right-up">
            <p>Account</p>
            <p class="account user name"><?php echo $username; ?></p>
        </div>
